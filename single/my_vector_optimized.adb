...
package body My_Vector is
   ...
   type Vector_Array is array (Natural range <>) of Vector_Type;
   ...
   procedure Iterate( This : in Object_Type ) is
      ...
      K : array( 1..N ) of Vector_Type := (others=>Null_Vector);
      ...
      begin
         ...
         DX := DT * X2(I).V + DT**2 / ( 2.0 * X1(I).Mass)* K(I);
         X2(I).X := X1(I).X + DX;
         X2(I).Mass := X1(I).Mass;
   ...
   begin
      ...
      for i in 1..N loop
         for j in 1..i loop
            declare
               F : constant Vector_type := Force(i,j);
            begin
               K(i) := K(i) + F;
               K(j) := K(j) - F;
            end ;
