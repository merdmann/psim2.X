...
with Parallel.Recursion.Work_Seeking_Sequential_Interleave;
...
package body FFT_Work_Seeking is
   ...
   is
      ...
      package Recursive_FFT is new
        Parallel.Recursion.Work_Seeking_Sequential_Interleave
      ...
      procedure Parallel_Danielson_Lanczos (Info : Work_Info_Type)
      is
         Minimum_Work_Offer : constant Count_Type := 2**13;
      ...
      begin
         ...
         if Other_Workers.Seeking_Work and then N > Minimum_Work_Offer then
            if Dispatcher.Recurse
              ((Offset, N / 2, Log2_N - 1, Barrier'Unchecked_Access)) then
               Parallel_Danielson_Lanczos
                 ((Offset + N, N / 2, Log2_N - 1, Barrier'Unchecked_Access));
            else
               Parallel_Danielson_Lanczos
                 ((Offset, N / 2, Log2_N - 1, null));
               Parallel_Danielson_Lanczos
                 ((Offset + N, N / 2, Log2_N - 1, null));
            end if;
         else
            Parallel_Danielson_Lanczos ((Offset, N / 2, Log2_N - 1, null));
            Parallel_Danielson_Lanczos ((Offset + N, N / 2, Log2_N - 1, null));
         end if;
   ...
   begin --  FFT_Work_Sharing
      ...
      Recursive_FFT.Execute
        (Item             => (Data'First, Data'Length / 2, Log2_N, null),
         ...
         Process          => Parallel_Danielson_Lanczos'Access,
