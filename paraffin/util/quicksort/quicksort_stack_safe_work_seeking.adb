...
--                         Q U I C K S O R T                             --
--            S T A C K - S A F E  W O R K - S E E K I N G             --
--                                                                          --
--                                S p e c                                   --
--                                                                          --
...
procedure Quicksort_Stack_Safe_Work_Seeking
...
is
   ...
   package Recursive_Quicksort is
     new Parallel.Recursion.Stack_Safe_Work_Seeking
   ...
   begin -- Quicksort
      ...
      if Bounds.Left < J then
         ...
         if Bounds'Address <= Stack_Limit or else (Other_Workers.Seeking_Work
           and then (Index_Type'Pos (J) -
                       Index_Type'Pos (Bounds.Left) > Minimum_Work_Offer)) then
            Dispatcher.Recurse ((Bounds.Left, J), Stack_Limit);
            Offered := True;
         else
            Quicksort ((Bounds.Left, J), Stack_Limit);
         end if;

      end if;

      if I < Bounds.Right then
         if Bounds'Address <= Stack_Limit or else
           (Other_Workers.Seeking_Work and then not Offered
            and then (Index_Type'Pos (J) - Index_Type'Pos (Bounds.Left)) >
              Minimum_Work_Offer) then

            Dispatcher.Recurse ((I, Bounds.Right), Stack_Limit);
         else
            Quicksort ((I, Bounds.Right), Stack_Limit);
         end if;
      end if;
   end Quicksort;
...
end Quicksort_Stack_Safe_Work_Seeking;
