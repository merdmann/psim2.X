...
with Parallel.Recursion.Work_Seeking_Functional_Reduction_With_Call_Depth;
use Parallel;
...
function Fibonacci_Elementary_Depth_Work_Seeking
  (Value : Natural) return Natural
is
   Other_Workers : aliased Parallel.Work_Seeking_State;

   package Fibonacci_Recursion is new
     Parallel.Recursion.Work_Seeking_Functional_Reduction_With_Call_Depth
       (Work_Type => Natural,
        Result_Type => Natural,
        Reducer => "+",
        Identity_Value => 0);
   ...
   function Parallel_Fibonacci (Number : Natural;
   ...
   begin
      ...
      if Number < 2 then
        ...
        Depth <= Parallel_Test_Harness.Fibonacci_Offer_Depth then
...
end Fibonacci_Elementary_Depth_Work_Seeking;
