...
package body Parallel.Recursion.Work_Sharing is
   ...
   procedure Execute
     (Item               : Work_Type;
      ...
      Sequential_Process : not null access procedure (Item : Work_Type);
   ...
   is
      ...
      begin -- Worker
         ...
         Work_Loop : loop
            ...
            if Subcontractors = 0 then
               ...
               Sequential_Process (Value);
               exit Work_Loop;
            else
               --  Store the remaining subcontract count so that it
               --  can be retrieved when client calls Recurse
               Parallel_Process (Value, Subcontractors);
            end if;
         end loop Work_Loop;
      ...
      begin
         ...
         Scheduler.Offer_Work (Item, Worker_Count, Work_Accepted);
         ...
         if Subcontractors > 0 then
            ...
            if Split = Of_Splits then
               ...
               if Subcontractor_Count <= 1 then
                  Sequential_Process (Item);
               else
                  --               Parallel_Attributes.Set_Value
                  --(Subcontractor_Count);
                  Parallel_Process (Item, Subcontractor_Count);
               end if;
